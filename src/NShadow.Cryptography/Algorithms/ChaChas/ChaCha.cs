...
using NShadow.Cryptography.Containers;

namespace NShadow.Cryptography.Algorithms.ChaChas
{
    public abstract class ChaCha : SymmetricAlgorithm
    {
        ...
        public static byte[] ComputeHash(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce)
        {
            ...
            var hashAsUInt = MemoryMarshal.Cast<byte, uint>(hash.AsSpan());

            var state = Initialize(key, nonce[ChaChaTransform.WordLength..], BinaryPrimitives.ReadUInt32LittleEndian(nonce[..ChaChaTransform.WordLength]));
            ...
            var preWordSlice = hashAsUInt[..ChaChaTransform.WordLength];
            var pastWordSlice = hashAsUInt[ChaChaTransform.WordLength..];

            stateSpan[..ChaChaTransform.WordLength].CopyTo(preWordSlice);
            stateSpan.Slice(ChaChaTransform.NumWordsPerBlock - ChaChaTransform.WordLength, ChaChaTransform.WordLength).CopyTo(pastWordSlice);
        ...
        public void Transform(Stream source, Stream destination)
        {
            ...
            try
            {
                ...
                var bufferSize = 4096;
                using var buffer = RecyclableByteArray.Rent(bufferSize);
                var numBytesRead = 0;
